...
"""
A minimal training script for NewBie-image-Exp0.1 using PyTorch FSDP.
"""
import argparse
...
from imgproc import generate_crop_size_list, to_rgb_if_rgba, center_crop
import models
...
class T2IItemProcessor(ItemProcessor):
    ...
    def process_item(self, data_item, training_mode=False):
        if "super_high_quality_caption" in data_item:
            ...
            #system_prompt = "You are an assistant designed to generate high-quality anime images with the highest degree of image-text alignment based on textual prompts. <Prompt Start> "
            system_prompt = "You are an assistant designed to generate high-quality anime images based on xml format prompts. <Prompt Start> "
        ...
        original_text = text
        text_with_system = system_prompt + text
        return image, text_with_system, original_text
...
def _choose_bucket_crop_size(pil_image, crop_size_list, random_top_k=1):
    w, h = pil_image.size
...
def dataloader_collate_fn(samples, image_transform=None, crop_size_list=None, random_top_k=1):
    image = [x[0] for x in samples]
    caps = [x[1] for x in samples]  
    original_caps = [x[2] for x in samples]  

    if crop_size_list is not None and len(image) > 0:
        bucket_size = _choose_bucket_crop_size(image[0], crop_size_list, random_top_k)
        image = [center_crop(img, bucket_size) for img in image]

    if image_transform is not None:
        image = [image_transform(img) for img in image]

    return image, caps, original_caps


def get_train_sampler(dataset, rank, world_size, global_batch_size, max_steps, resume_step, seed):
    local_bsz = global_batch_size // world_size
    total_per_rank = max_steps * local_bsz

    if (
        not hasattr(dataset, "bucket_indices")
        or dataset.bucket_indices is None
        or len(dataset.bucket_indices) == 0
    ):
    ...
    bucket_ids = sorted(dataset.bucket_indices.keys())

    def reset_bucket_state():
        state = {}
        for b in bucket_ids:
            idxs = dataset.bucket_indices.get(b, [])
            if not idxs:
                continue
            rank_indices = idxs[rank::world_size]
            if len(rank_indices) == 0:
                continue
            perm = torch.randperm(len(rank_indices), generator=g).tolist()
            shuffled = [rank_indices[i] for i in perm]
            state[b] = shuffled
        return state

    buckets_state = reset_bucket_state()
    if len(buckets_state) == 0:
        sample_indices = torch.randint(0, len(dataset), (total_per_rank,), generator=g, dtype=torch.long)
        return sample_indices[resume_step * local_bsz :].tolist()

    sample_indices = []
    while len(sample_indices) < total_per_rank:
        candidates = [b for b, idxs in buckets_state.items() if len(idxs) >= local_bsz]
        if not candidates:
            buckets_state = reset_bucket_state()
            candidates = [b for b, idxs in buckets_state.items() if len(idxs) >= local_bsz]
            if not candidates:
                break
        lengths = torch.tensor([len(buckets_state[b]) for b in candidates], dtype=torch.float)
        probs = lengths / lengths.sum()
        choice = torch.multinomial(probs, 1, generator=g).item()
        b = candidates[choice]
        batch = buckets_state[b][:local_bsz]
        buckets_state[b] = buckets_state[b][local_bsz:]
        sample_indices.extend(batch)

    if len(sample_indices) < total_per_rank:
        extra = total_per_rank - len(sample_indices)
        all_indices = []
        for idxs in buckets_state.values():
            all_indices.extend(idxs)
        if len(all_indices) > 0:
            perm = torch.randperm(len(all_indices), generator=g).tolist()
            all_indices = [all_indices[i] for i in perm]
            sample_indices.extend(all_indices[:extra])
        if len(sample_indices) < total_per_rank:
            remaining = total_per_rank - len(sample_indices)
            rand_idx = torch.randint(0, len(dataset), (remaining,), generator=g, dtype=torch.long).tolist()
            sample_indices.extend(rand_idx)

    sample_indices = sample_indices[:total_per_rank]
    return sample_indices[resume_step * local_bsz :].tolist()
...
def setup_lm_fsdp_sync(model: nn.Module) -> FSDP:
    ...
    return model
def setup_clip_fsdp_sync(model: nn.Module) -> FSDP:
    # LM FSDP always use FULL_SHARD among the node.
    print(model)
    model = FSDP(
        model,
...
def encode_prompt(prompt_batch, text_encoder, tokenizer, clip_model, clip_tokenizer, proportion_empty_prompts, is_train=True, clip_captions=None):
    captions = []
    ...
    with torch.no_grad():
        ...
        clip_text_pooled = clip_model.get_text_features(input_ids=clip_inputs)

        if clip_text_pooled is not None:
            clip_text_pooled = clip_text_pooled.clone()
            clip_mask = clip_inputs.attention_mask

    return prompt_embeds, prompt_masks, None, clip_text_pooled, clip_mask
...
def main(args):
    ...
    logger.info(f"Setting-up CLIP model: jinaai/jina-clip-v2")
    ...
    #logger.info("Disabling LoRA adapters on the loaded CLIP model...")

   # clip_config.text_config.hf_model_config_kwargs['lora_adaptations'] = []
    #clip_config.text_config.hf_model_config_kwargs['lora_rank'] = 0
...
#    clip_model = setup_clip_fsdp_sync(clip_model)
#    logger.info(f"CLIP model: {type(clip_model)}")
    ...
    logger.info(f'clip norm {model.time_text_embed[1].weight.norm()}')
    ...
    #opt = Muon(list(model.parameters()), lr=args.lr, weight_decay=args.wd)
    from torch.optim.lr_scheduler import CosineAnnealingLR, LinearLR

    warmup_scheduler = LinearLR(opt, start_factor=1e-6, total_iters=args.warmup_steps)
    cosine_scheduler = CosineAnnealingLR(opt, T_max=args.max_steps - args.warmup_steps, eta_min=1e-6)

    # Chain the schedulers
    scheduler = torch.optim.lr_scheduler.SequentialLR(
        opt, schedulers=[warmup_scheduler, cosine_scheduler], milestones=[args.warmup_steps]
    )
    if args.resume:
        ...
        scheduler_path = os.path.join(args.resume, "scheduler.pth")
        if os.path.exists(scheduler_path):
            logger.info(f"Resuming scheduler states from: {scheduler_path}")
            scheduler.load_state_dict(torch.load(scheduler_path, map_location="cpu"))
        else:
            logger.warning(f"Scheduler state not found at {scheduler_path}, starting with a new scheduler.")
    ...
    for train_res in [1024]:
        ...
        dataset.assign_buckets(crop_size_list, random_top_k=1)
    ...
    for step in range(resume_step, args.max_steps):
        ...
        for train_res, data_pack in data_collection.items():
            x, caps, original_caps = next(data_pack["loader_iter"])
            ...
            with torch.no_grad():
                ...
                for idx, img in enumerate(x):
                    ...
                    shape2indices.setdefault((h, w), []).append(idx)

                latents = [None] * len(x)
                for (h, w), idxs in shape2indices.items():
                    batch = torch.stack([x[i] for i in idxs], dim=0)  # [B_hw, 3, h, w]
                    batch_latents = vae.encode(batch.bfloat16()).latent_dist.mode()  # [B_hw, C, H', W']
                    batch_latents = (batch_latents - vae_shift) * vae_scale
                    batch_latents = batch_latents.float()
                    for j, idx in enumerate(idxs):
                        latents[idx] = batch_latents[j]

                x = latents

            with torch.no_grad():
                cap_feats, cap_mask, clip_text_sequence, clip_text_pooled, clip_mask = encode_prompt(
                    caps, text_encoder, tokenizer, clip_model, clip_tokenizer, args.caption_dropout_prob,
                    clip_captions=original_caps
                )
            ...
            for mb_idx in range((data_pack["local_bsz"] - 1) // data_pack["micro_bsz"] + 1):
                ...
                model_kwargs = dict(cap_feats=cap_feats_mb,
                                    cap_mask=cap_mask_mb,
                                    clip_text_sequence=None,
                                    clip_text_pooled=clip_text_pooled[mb_st:mb_ed],
        ...
        if step == 0 or (step + 1) % args.ckpt_every == 0 or (step + 1) == args.max_steps:
            ...
            with FSDP.state_dict_type(
                ...
                if dist.get_rank() == 0:
                    scheduler_state_fn = "scheduler.pth"
                    torch.save(scheduler.state_dict(), os.path.join(checkpoint_path, scheduler_state_fn))
            dist.barrier()
            logger.info(f"Saved optimizer and scheduler to {checkpoint_path}.")
